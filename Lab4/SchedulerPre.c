...
uint8_t Activate(uint8_t Prio, uint16_t Ticks) {
  if (Prio>=NUMTASKS) return (E_BOUNDS);

  Taskp CurTask = Prio2Taskp(Prio);

  if (Ticks == 0) { //Activation is supposed to be now
    if (CurTask->Flags & DIRECT) {CurTask->Activated++; CurTask->Invoked++; CurTask->Taskf(); } // DIRECT task detected, so execute it right away
    else { CurTask->Flags |= TRIGGERED; CurTask->Activated++; }
  }
  if (Ticks > 0) { //Activation is supposed to be somewhere in the future
    if (Ticks < CurTask->Remaining || CurTask->Remaining == 0) { CurTask->Remaining = Ticks; CurTask->Flags |= TT;} //if Activation is not already scheduled, do so
  }
